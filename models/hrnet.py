...
# Temporarily replace with standard BatchNorm2d
...
def get_lip_config():
    """
    Get the configuration for the HRNet-OCR model trained on the LIP dataset.
    """
    ...
    cfg.MODEL.PRETRAINED = '' # This will be loaded from the user's path
    cfg.DATASET.NUM_CLASSES = 20

    cfg.MODEL.EXTRA = types.SimpleNamespace()
    cfg.MODEL.EXTRA.STAGE1 = {
        'NUM_MODULES': 1,
        'NUM_BRANCHES': 1,
        'BLOCK': 'BOTTLENECK',
        'NUM_BLOCKS': [4],
    ...
    cfg.MODEL.OCR = types.SimpleNamespace()
    cfg.MODEL.OCR.MID_CHANNELS = 512
    cfg.MODEL.OCR.KEY_CHANNELS = 256

    return cfg

def segment_person(image_path, weights_path):
    """
    Segments a person in an image using the HRNet-OCR model.
    ...
    pred = torch.argmax(output[1], dim=1).squeeze(0).cpu().numpy()
    
    palette = get_palette(20) # 20 classes for LIP dataset
