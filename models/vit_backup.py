...
if __name__ == '__main__':
    ...
    try:
        ...
        # Number of parameters in the model loaded
        num_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        print(f"Total trainable parameters: {num_params}")

        #number of parameters in the model architecture
        num_params_arch = sum(p.numel() for p in model.parameters())
        print(f"Total parameters in the architecture: {num_params_arch}")

        # Verify the model by running a forward pass with a dummy input
        print("\n--- Verifying the model ---")
        # number of mismatched keys
        mismatched_keys = [k for k in model.state_dict().keys() if k not in checkpoint]
        if mismatched_keys:
            print(f"[WARNING] Mismatched keys found: {len(mismatched_keys)}")
            print("Mismatched keys:", mismatched_keys)
        else:
            print("[SUCCESS] No mismatched keys found.")
            
        model.eval()
        print("\nModel set to evaluation mode.")

        with torch.no_grad():
            # Use the correct non-square input size
